...
{
    /// <summary>
    /// Receives the stream make a que of all the received packets, and process once final packet is received
    /// </summary>
    public class StreamReceiverProcessOnce : BaseNotifyPropertyChanged, IStreamReceiver
    {
        ...
        public StreamReceiverProcessOnce(Camera camera)
        {
            Camera = camera;
        ...
        private int bufferIndex = 0, packetRxCountClone, bufferIndexClone;
        private ulong imageID, lastImageID = 0, lastImageIDClone, deltaImageID, frameCounter;
        ...
        private async void DecodePacketsRawSocket()
        {
            try
            {
                ...
                Memory<byte> bufferSinglePacket1 = new byte[Camera.rawBytes.Length * 4];
